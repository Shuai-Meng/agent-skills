...
## Scope (per-project split)
- Facts tied to ONE project (tech stack, branch, project decisions) go into that project's AGENTS.md (create if missing), NOT the global MEMORY.md.
- Global MEMORY.md is only for cross-project facts: personal prefs, global tooling/config, decisions not tied to one repo.
- When working inside a project, check its AGENTS.md at session start too.
